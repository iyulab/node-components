 ...
 *
 * ✅**`u-input` 이 암묵 제출을 흉내 내게 된 뒤로 이 파일은 그 기능의 가드다** — 입력은 Enter 를 받고 한 틱
 * 뒤에 제출을 판정하므로, 여기서 폼의 `keydown` 을 취소하면 제출이 없어야 한다. 판정을 마이크로태스크로 당기면
 * (폼 리스너보다 먼저 돈다) 아래 두 `u-input` 케이스가 빨개진다(`input-implicit-submit.browser.test.ts` 참조).
 *
