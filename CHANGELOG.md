...
- **`u-checkbox` 의 `variant × color × state` 매트릭스를 접었다** — 16개 → 8개. 이 컴포넌트는 이미 `--checkbox-fill-color` 채움색 훅을 갖고 있었고, 16개 규칙이 그것을 우회해 팔레트를 직접 쓰고 있었다. 이제 `color=` 는 훅 하나만 바꾸고 variant 규칙이 그것을 소비한다.
  - 기본값 `blue` 는 규칙을 갖지 않는다 = 브랜드 훅을 그대로 탄다. 종전에도 `[color="blue"]` 규칙은 `--checkbox-fill-color` 를 재진술만 하고 있었다.
...
> **이 버전은 1.12.0 에 포함되어 함께 출하됐다 — 별도 게시본은 없다.**
