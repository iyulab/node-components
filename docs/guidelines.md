...
- 메시지 조회 순서: `Locale.getValue()`가 찾는 값(활성 로케일 → 접두를 줄인 태그 → 그 언어의 기본 지역형 → 영어). 기본 지역형은 내장 표가 지역형으로만 있는 언어(`zh`·`pt`)를 위한 것으로 `REGIONAL_DEFAULTS` 에 명시한다 — 중국어는 표기 체계가 둘이라 «첫 번째 표» 로 고르지 않는다.
